...
/**
 * Ensure private key files are readable by non-root users (uid 1000).
 * The tmux-worker container runs as the `node` user and needs read access
 * to the key files generated by this root-owned init container.
 * Issue #1979
 */
function fixKeyPermissions(keyFiles) {
  for (const keyFile of keyFiles) {
    if (fs.existsSync(keyFile)) {
      try {
        fs.chmodSync(keyFile, 0o644);
      } catch (err) {
        console.warn(`  Warning: could not chmod ${keyFile}: ${err.message}`);
      }
    }
  }
}
...
function main() {
  ...
  const KEY_FILES = [caKeyPath, apiKeyPath, workerKeyPath];

  // Skip generation if certs already exist — but always fix permissions
  // so existing deployments with 0600 root-owned keys become readable by
  // the non-root tmux-worker container (uid 1000).  Issue #1979.
  ...
  fs.writeFileSync(caKeyPath, caKeyPair.privateKey, { mode: 0o644 });
  ...
  fs.writeFileSync(apiKeyPath, apiKeyPair.privateKey, { mode: 0o644 });
  ...
  fs.writeFileSync(workerKeyPath, workerKeyPair.privateKey, { mode: 0o644 });
