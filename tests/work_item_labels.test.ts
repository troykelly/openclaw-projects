...
describe('Work Item Labels (Issue #221)', () => {
  ...
  describe('Label table', () => {
    it('creates labels with auto-normalized name', async () => {
      const result = await pool.query(`INSERT INTO label (name) VALUES ('Shopping') RETURNING name, normalized_name`);
    ...
    it('enforces unique normalized names', async () => {
      ...
      await expect(pool.query(`INSERT INTO label (name) VALUES ('SHOPPING')`)).rejects.toThrow(/unique|duplicate/i);
    });
  ...
  describe('Work item cascade delete', () => {
    it('removes label associations when work item is deleted', async () => {
      ...
      const labels = await pool.query(`SELECT COUNT(*) as count FROM label WHERE normalized_name = 'cascade-test'`);
      expect(parseInt(labels.rows[0].count, 10)).toBe(1);
    });
  });
  ...
  describe('Label cascade delete', () => {
    it('removes associations when label is deleted', async () => {
      ...
      const associations = await pool.query(`SELECT COUNT(*) as count FROM work_item_label WHERE label_id = $1`, [labelId]);
  ...
  describe('Querying work items by labels', () => {
    it('filters work items by single label', async () => {
      ...
      const wi1 = await pool.query(`INSERT INTO work_item (title) VALUES ('Home Task') RETURNING id::text as id`);
      const wi2 = await pool.query(`INSERT INTO work_item (title) VALUES ('Work Task') RETURNING id::text as id`);
      const wi3 = await pool.query(`INSERT INTO work_item (title) VALUES ('Home and Work') RETURNING id::text as id`);
    ...
    it('filters work items by multiple labels (AND)', async () => {
      const wi1 = await pool.query(`INSERT INTO work_item (title) VALUES ('Single Label') RETURNING id::text as id`);
      const wi2 = await pool.query(`INSERT INTO work_item (title) VALUES ('Both Labels') RETURNING id::text as id`);
  ...
  describe('Label usage counts', () => {
    it('can count work items per label', async () => {
      const wi1 = await pool.query(`INSERT INTO work_item (title) VALUES ('WI 1') RETURNING id::text as id`);
      const wi2 = await pool.query(`INSERT INTO work_item (title) VALUES ('WI 2') RETURNING id::text as id`);
      const wi3 = await pool.query(`INSERT INTO work_item (title) VALUES ('WI 3') RETURNING id::text as id`);
