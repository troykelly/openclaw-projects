...
describe('Memory Service', () => {
  ...
  describe('createMemory', () => {
    it('creates a global memory', async () => {
      ...
      expect(memory.embedding_status).toBe('pending');
    });
    ...
    it('creates a memory with work item scope', async () => {
      ...
      expect(memory.work_item_id).toBe(work_item_id);
    ...
    it('creates a memory with contact scope', async () => {
      ...
      expect(memory.contact_id).toBe(contact_id);
    ...
    it('creates a memory with full attribution', async () => {
      const memory = await createMemory(pool, {
        ...
        source_url: 'https://example.com/conversation/123',
      });

      expect(memory.created_by_agent).toBe('openclaw-pi');
      expect(memory.created_by_human).toBe(false);
      expect(memory.source_url).toBe('https://example.com/conversation/123');
    });
    ...
    it('creates a memory with expiry', async () => {
      ...
      expect(Math.abs(memory.expires_at!.getTime() - expires_at.getTime())).toBeLessThan(1000);
    });
  ...
  describe('updateMemory', () => {
    it('updates memory title and content', async () => {
      ...
      expect(updated!.embedding_status).toBe('pending'); // Reset on content change
    });
  ...
  describe('listMemories', () => {
    it('lists all memories', async () => {
      await createMemory(pool, { user_email: 'test@example.com', title: 'Memory 1', content: 'Content 1' });
      await createMemory(pool, { user_email: 'test@example.com', title: 'Memory 2', content: 'Content 2' });
    ...
    it('filters by memory type', async () => {
      await createMemory(pool, { user_email: 'test@example.com', title: 'Pref', content: 'C1', memory_type: 'preference' });
      await createMemory(pool, { user_email: 'test@example.com', title: 'Fact', content: 'C2', memory_type: 'fact' });

      const result = await listMemories(pool, { memory_type: 'preference' });
    ...
    it('respects pagination', async () => {
      for (let i = 0; i < 5; i++) {
        await createMemory(pool, { user_email: 'test@example.com', title: `Memory ${i}`, content: `Content ${i}` });
      }

      const result = await listMemories(pool, { user_email: 'test@example.com', limit: 2, offset: 1 });
  ...
  describe('supersedeMemory', () => {
    it('creates new memory and marks old as superseded', async () => {
      ...
      expect(oldUpdated!.superseded_by).toBe(newMemory.id);
    });
  });
  ...
  describe('searchMemories', () => {
    ...
    it('falls back to text search when no embeddings match', async () => {
      ...
      expect(result.search_type).toBeDefined();
    });
    ...
    it('filters search by memory type', async () => {
      ...
      const result = await searchMemories(pool, 'pizza', { memory_type: 'preference' });

      // Should filter to preference type only
      if (result.results.length > 0) {
        expect(result.results[0].memory_type).toBe('preference');
      }
      // At minimum, verify no facts are returned
      expect(result.results.every((r) => r.memory_type !== 'fact')).toBe(true);
    });
  });
});
