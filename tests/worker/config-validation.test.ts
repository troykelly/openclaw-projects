...
describe('validateOpenClawConfig', () => {
  ...
  describe('valid configuration', () => {
    it('returns valid when all vars are properly set', () => {
      ...
      process.env.OPENCLAW_HOOK_TOKEN = 'my-secret-token';
  ...
  describe('missing variables', () => {
    ...
    it('reports missing OPENCLAW_HOOK_TOKEN', () => {
      process.env.OPENCLAW_GATEWAY_URL = 'https://gateway.example.com';
      delete process.env.OPENCLAW_HOOK_TOKEN;

      const result = validateOpenClawConfig();
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('OPENCLAW_HOOK_TOKEN is not set');
    });
  ...
  describe('invalid values', () => {
    ...
    it('detects 1Password reference (op://)', () => {
      process.env.OPENCLAW_GATEWAY_URL = 'https://gateway.example.com';
      process.env.OPENCLAW_HOOK_TOKEN = 'op://vault/item/field';
    ...
    it('detects 1Password CLI placeholder', () => {
      process.env.OPENCLAW_GATEWAY_URL = 'https://gateway.example.com';
      process.env.OPENCLAW_HOOK_TOKEN = "some text [use 'op item get' to retrieve] rest";
