...
describe('Security Tests', () => {
  ...
  describe('Error Message Sanitization', () => {
    it('should not expose internal paths in error messages', async () => {
      const mockGet = vi.fn().mockRejectedValue(new Error('ENOENT: no such file or directory, open /internal/path/to/secret'));
      const client = { ...mockApiClient, get: mockGet };
    ...
    it('should not expose database connection strings in errors', async () => {
      const mockGet = vi.fn().mockRejectedValue(new Error('Connection failed: postgres://user:password@localhost/db'));
      const client = { ...mockApiClient, get: mockGet };
