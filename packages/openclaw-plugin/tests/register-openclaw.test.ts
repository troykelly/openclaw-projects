...
describe('OpenClaw 2026 API Registration', () => {
  ...
  describe('registration', () => {
    it('should register all 109 tools', () => {
      registerOpenClaw(mockApi);

      expect(registeredTools).toHaveLength(109);
      ...
      // Issue #2342: notebook_delete
      expect(toolNames).toContain('notebook_delete');
      // Issue #1923: Tool guide meta-tool
      expect(toolNames).toContain('tool_guide');
      // Epic #2011, Issue #2015: Dev prompt tools
      expect(toolNames).toContain('dev_prompt_list');
      expect(toolNames).toContain('dev_prompt_get');
      expect(toolNames).toContain('dev_prompt_create');
      expect(toolNames).toContain('dev_prompt_update');
      expect(toolNames).toContain('dev_prompt_reset');
    });

    it('should mark 67 tools as optional', () => {
      registerOpenClaw(mockApi);

      const optionalTools = registeredTools.filter((t) => t.optional === true);
      expect(optionalTools).toHaveLength(68);
    ...
    it('should use the user prompt from event for semantic search', async () => {
      ...
      globalThis.fetch = vi.fn().mockImplementation(async (url: string) => {
        ...
        return {
          ...
          status: 200,
          json: async () => ({
            results: [{ id: '1', content: 'User prefers sushi', type: 'preference', score: 0.9 }],
          }),
        };
  ...
  describe('tool definitions', () => {
    ...
    it('should have execute functions with correct OpenClaw Gateway signature', async () => {
      ...
      globalThis.fetch = vi.fn().mockImplementation(async () => ({
        ...
        status: 200,
        json: async () => ({
          results: [{ id: '1', content: 'test', type: 'fact', score: 0.9 }],
        }),
      })) as unknown as typeof fetch;
