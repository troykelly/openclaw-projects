 ...
 * Verifies cross-entity recall that fans out to memories, todos, and projects.
 *
 * Part of Issue #1219.
 */
...
describe('context_search tool', () => {
  ...
  describe('fan-out to APIs', () => {
    function createMockClient(overrides?: { memoriesResponse?: unknown; searchResponse?: unknown }) {
    ...
    it('should call both /api/memories/search and /api/search when entity_types omitted', async () => {
      ...
      expect(calls.some((url: string) => url.includes('/api/search'))).toBe(true);
    });
  ...
  describe('score normalization and merging', () => {
    function createMockClient(memoriesResults: unknown[], searchResults: unknown[]) {
      const mockGet = vi.fn().mockImplementation((url: string) => {
        if (url.includes('/api/memories/search')) {
