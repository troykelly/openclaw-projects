...
import { resolveSecret, resolveSecretSync, resolveSecrets, clearSecretCache, type SecretConfig } from '../src/secrets.js';
  ...
  describe('resolveSecret', () => {
    ...
    describe('file reference', () => {
      ...
      it('should expand ~ in file path', async () => {
        ...
        const result = await resolveSecret(config);
        expect(result).toBe('secret-from-file');
        expect(fs.readFileSync).toHaveBeenCalledWith(path.join(homeDir, '.secrets', 'api_key'), 'utf-8');
      });
      ...
      it('should warn when file is world-readable', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('world-readable'));
      });
      ...
      it('should log warning when statSync throws (not silent)', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Could not check permissions'));
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('ELOOP'));
      });
    ...
    describe('command reference', () => {
      ...
      it('should use default timeout of 5 seconds', async () => {
        ...
        await resolveSecret(config);
        expect(childProcess.execSync).toHaveBeenCalledWith('some-command', expect.objectContaining({ timeout: 5000 }));
      });
      ...
      it('should allow custom timeout', async () => {
        ...
        await resolveSecret(config);
        expect(childProcess.execSync).toHaveBeenCalledWith('some-command', expect.objectContaining({ timeout: 10000 }));
      });
