  ...
  return {
    ...
    async execute(params: TodoListParams): Promise<TodoListResult> {
      ...
      try {
        ...
        const response = await client.get<{ items?: WorkItemRow[]; total?: number }>(`/work-items?${queryParams.toString()}`, { user_id });
  ...
  return {
    ...
    async execute(params: TodoCreateParams): Promise<TodoCreateResult> {
      ...
      try {
        ...
        const response = await client.post<{ id: string; title?: string }>('/work-items', body, { user_id });
...
export function createTodoCompleteTool(options: TodoToolOptions): TodoCompleteTool {
  ...
  return {
    ...
    async execute(params: TodoCompleteParams): Promise<TodoCompleteResult> {
      ...
      try {
        const response = await client.patch<{ status: string; title?: string }>(`/work-items/${id}/status`, { status: 'completed' }, { user_id });
