...
function formatAggregateResult(operation: string, result: Record<string, unknown>): string {
  switch (operation) {
    ...
    case 'count_by_tag': {
      ...
      return (
        tags
          .filter((t): t is { tag: string; count: number } => t != null && typeof t === 'object' && 'tag' in t && 'count' in t)
          .map((t) => `- ${t.tag}: ${t.count}`)
          .join('\n') || 'No tags found.'
      );
    }
    ...
    case 'count_by_status': {
      ...
      return (
        statuses
          .filter((s): s is { status: string; count: number } => s != null && typeof s === 'object' && 'status' in s && 'count' in s)
