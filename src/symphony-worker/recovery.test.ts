...
describe('recoverOrphanedRuns', () => {
  ...
  it('transitions claimed runs to failed', async () => {
    const pool = createMockPool();
  ...
  it('transitions running runs to stalled', async () => {
    const pool = createMockPool();
    pool.query.mockResolvedValueOnce({
      rows: [{ id: 'run-2', status: 'running' }],
      rowCount: 1,
    });
    pool.query.mockResolvedValueOnce({ rowCount: 1 });
    pool.query.mockResolvedValueOnce({ rowCount: 1 });
    ...
    const results = await recoverOrphanedRuns(pool as never, 'test-ns', 'stale-orch-1');
    expect(results).toHaveLength(1);
    expect(results[0].newStatus).toBe('stalled');
  });
  ...
  it('handles multiple orphaned runs', async () => {
    ...
    expect(results[0].newStatus).toBe('failed');
    expect(results[1].newStatus).toBe('stalled');
    expect(results[2].newStatus).toBe('orphaned');
  });
