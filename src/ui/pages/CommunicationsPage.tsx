...
import { Mail, Calendar, MessageSquare, Search, SlidersHorizontal, ArrowUpDown, X, Link2, Unlink2, ChevronDown } from 'lucide-react';
 ...
 */
function buildCommunicationItems(emails: LinkedEmail[], events: LinkedCalendarEvent[], sortOrder: SortOrder): CommunicationItem[] {
  ...
  items.sort((a, b) => (sortOrder === 'newest' ? b.date.getTime() - a.date.getTime() : a.date.getTime() - b.date.getTime()));
...
export function CommunicationsPage({
  ...
  const hasError = !hasPropsEmails && !hasPropsEvents && (emailsQuery.error != null || eventsQuery.error != null);
  const errorMessage = (emailsQuery.error as Error | null)?.message ?? (eventsQuery.error as Error | null)?.message ?? 'An unexpected error occurred';
  ...
  // Filter data
  const { filteredEmails, filteredEvents } = useMemo(() => filterBySearch(emails, calendarEvents, searchQuery), [emails, calendarEvents, searchQuery]);

  // Sorted email list
  const sortedEmails = useMemo(() => {
    const sorted = [...filteredEmails];
    sorted.sort((a, b) => (sortOrder === 'newest' ? b.date.getTime() - a.date.getTime() : a.date.getTime() - b.date.getTime()));
    return sorted;
  ...
  const sortedEvents = useMemo(() => {
    ...
    sorted.sort((a, b) => (sortOrder === 'newest' ? b.startTime.getTime() - a.startTime.getTime() : a.startTime.getTime() - b.startTime.getTime()));
    return sorted;
  }, [filteredEvents, sortOrder]);

  // Combined list for "All" tab
  const allItems = useMemo(() => buildCommunicationItems(filteredEmails, filteredEvents, sortOrder), [filteredEmails, filteredEvents, sortOrder]);
  ...
  return (
    <div data-testid="page-communications" className="flex h-full flex-col p-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => setSortOrder('newest')} className={cn(sortOrder === 'newest' && 'font-medium')}>
                Newest first
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortOrder('oldest')} className={cn(sortOrder === 'oldest' && 'font-medium')}>
                Oldest first
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          {/* Filter toggle */}
          <Button variant={showFilters ? 'secondary' : 'outline'} size="sm" onClick={() => setShowFilters((prev) => !prev)} data-testid="filter-toggle">
      ...
      {/* Filter bar */}
      {showFilters && (
        <div data-testid="filter-bar" className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border bg-muted/30 p-3">
      ...
      {/* Tabs */}
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as CommunicationTab)} className="flex min-h-0 flex-1 flex-col">
        ...
        <TabsContent value="all" className="mt-4 flex-1">
          {allItems.length > 0 ? (
              ...
              <div className="space-y-2 pr-4" data-testid="communications-list-all">
                {allItems.map((item) =>
                  item.kind === 'email' && item.email ? (
                    <EmailCard key={`email-${item.id}`} email={item.email} onClick={handleEmailClick} />
                  ) : item.event ? (
                    <CalendarEventCard key={`event-${item.id}`} event={item.event} onClick={handleEventClick} />
                  ) : null,
                )}
              </div>
            </ScrollArea>
            ...
            <EmptyState
              ...
              description={
                searchQuery ? 'Try adjusting your search terms or filters.' : 'Emails and calendar events linked to your work items will appear here.'
              }
            />
          )}
        </TabsContent>
        ...
        <TabsContent value="emails" className="mt-4 flex-1">
          {sortedEmails.length > 0 ? (
            ...
            <EmptyState
              ...
              description={searchQuery ? 'Try adjusting your search terms.' : 'Emails linked to your work items will appear here.'}
            />
          )}
        </TabsContent>
        ...
        <TabsContent value="calendar" className="mt-4 flex-1">
          {sortedEvents.length > 0 ? (
            <ScrollArea className="h-[calc(100vh-380px)]">
              <div className="space-y-2 pr-4" data-testid="communications-list-calendar">
                {sortedEvents.map((event) => (
                  <CalendarEventCard key={event.id} event={event} onClick={handleEventClick} />
                ))}
              </div>
            </ScrollArea>
          ) : (
            <EmptyState
              ...
              description={searchQuery ? 'Try adjusting your search terms.' : 'Calendar events linked to your work items will appear here.'}
            />
          )}
        </TabsContent>
      </Tabs>
      ...
      {/* Detail sheets */}
      <EmailDetailSheet email={selectedEmail} open={emailDetailOpen} onOpenChange={setEmailDetailOpen} />
      <CalendarEventDetailSheet event={selectedEvent} open={eventDetailOpen} onOpenChange={setEventDetailOpen} />
    </div>
  );
}
