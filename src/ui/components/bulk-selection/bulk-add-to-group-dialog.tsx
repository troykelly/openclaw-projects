...
export function BulkAddToGroupDialog({ open, onOpenChange, selectedCount, groups, onConfirm, loading = false }: BulkAddToGroupDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>Select a group to add the selected contacts to.</DialogDescription>
        </DialogHeader>
        ...
        <ScrollArea className="max-h-64 border rounded-md">
          <div className="p-2 space-y-1">
            ...
            {groups.length === 0 && <div className="py-4 text-center text-sm text-muted-foreground">No groups available</div>}
          </div>
        </ScrollArea>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={loading || !selectedGroupId}>
            {loading ? (
              <>
