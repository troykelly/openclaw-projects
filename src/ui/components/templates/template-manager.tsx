...
export function TemplateManager({ className }: TemplateManagerProps) {
  ...
  const filteredTemplates = React.useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter((t) => t.name.toLowerCase().includes(searchLower) || t.description.toLowerCase().includes(searchLower));
    }
        ...
        <div className="p-4 space-y-3">
          {filteredTemplates.length === 0 ? (
            ...
            filteredTemplates.map((template) => (
              <div key={template.id} data-template={template.id} className="p-4 rounded-lg border bg-card">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="font-medium">{template.name}</h4>
                      <span className="text-xs bg-muted px-2 py-0.5 rounded">{CATEGORY_LABELS[template.category]}</span>
                      {template.isBuiltIn && <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded">Built-in</span>}
                    </div>
                ...
                <div className="mt-3 text-xs text-muted-foreground">
                  ...
                  {template.structure.children && template.structure.children.length > 0 && (
                    <span className="ml-2">• {countTotalItems(template.structure)} items total</span>
                  )}
                  {template.structure.todos && template.structure.todos.length > 0 && <span className="ml-2">• {template.structure.todos.length} todos</span>}
                </div>
              </div>
            ))
          )}
...
function countTotalItems(structure: WorkItemTemplate['structure'], count = 1): number {
