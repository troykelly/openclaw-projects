...
import { RELATIONSHIP_TYPES, CATEGORY_LABELS, getRelationshipLabel, CATEGORY_COLORS, STRENGTH_LABELS } from './relationship-utils';
...
export function RelationshipFilter({ selectedTypes, selectedStrengths, onTypeChange, onStrengthChange, className }: RelationshipFilterProps) {
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      <div className="space-y-3">
        ...
        {(Object.entries(RELATIONSHIP_TYPES) as [RelationshipCategory, RelationshipType[]][]).map(([category, types]) => (
          <div key={category}>
            <div className="text-xs font-medium mb-1">{CATEGORY_LABELS[category]}</div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? CATEGORY_COLORS[category] : 'bg-muted hover:bg-muted/80 text-muted-foreground',
                    )}
          ...
          {(['strong', 'medium', 'weak'] as RelationshipStrength[]).map((strength) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'px-2 py-0.5 text-xs rounded-full transition-colors',
                  isSelected ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80 text-muted-foreground',
                )}
