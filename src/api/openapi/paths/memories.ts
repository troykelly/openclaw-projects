    ...
    schemas: {
      Memory: {
        ...
        properties: {
          ...
          importance: { type: 'number', nullable: true, description: 'Importance score: 0-1 float (normalized to 1-10) or 1-10 integer', example: 0.8 },
      ...
      LegacyMemory: {
        ...
        properties: {
          ...
          linked_item_id: { type: 'string', format: 'uuid', nullable: true, description: 'UUID of the linked work item (null if unlinked)', example: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890' },
          linked_item_title: { type: 'string', nullable: true, description: 'Title of the linked work item (null if unlinked)', example: 'Q1 Sprint Planning' },
      ...
      UnifiedMemoryCreateInput: {
        ...
        properties: {
          ...
          importance: { type: 'number', minimum: 0, maximum: 10, description: 'Importance score: 0-1 float (normalized to 1-10) or 1-10 integer', example: 0.8 },
    ...
    paths: {
      ...
      '/memory': {
        ...
        post: {
          ...
          requestBody: jsonBody({
            ...
            required: ['content'],
            properties: {
              title: { type: 'string', description: 'Short title for the memory (auto-generated from content if omitted)', example: 'Sprint planning decisions' },
              content: { type: 'string', description: 'Full text content of the memory', example: 'Team decided to focus on authentication module first' },
              linked_item_id: { type: 'string', format: 'uuid', description: 'UUID of the work item to link this memory to (optional)', example: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890' },
              work_item_id: { type: 'string', format: 'uuid', description: 'Alias for linked_item_id', example: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890' },
              contact_id: { type: 'string', format: 'uuid', description: 'UUID of the contact to link this memory to', example: 'b2c3d4e5-f6a7-8901-bcde-f12345678901' },
              ...
              memory_type: { type: 'string', enum: ['preference', 'fact', 'note', 'decision', 'context', 'reference', 'entity', 'other'], default: 'note', description: 'Alias for type', example: 'decision' },
              tags: { type: 'array', items: { type: 'string' }, description: 'Tags for categorizing the memory', example: ['sprint', 'planning'] },
              importance: { type: 'number', minimum: 0, maximum: 10, description: 'Importance score: 0-1 float (normalized to 1-10) or 1-10 integer', example: 0.8 },
              confidence: { type: 'number', minimum: 0, maximum: 1, description: 'Confidence score (0-1)', example: 0.95 },
              expires_at: { type: 'string', format: 'date-time', nullable: true, description: 'Expiration timestamp', example: '2026-12-31T23:59:59Z' },
              source_url: { type: 'string', nullable: true, description: 'Source URL', example: 'https://docs.example.com' },
            },
          }),
          responses: {
        ...
        get: {
          ...
          parameters: [
            ...
            },
            {
              name: 'namespaces',
              in: 'query',
              description: 'Comma-separated list of namespaces to search across',
              schema: { type: 'string' },
              example: 'default,project-a',
            },
          ],
          responses: {
        ...
        post: {
          ...
          requestBody: jsonBody({
            ...
            properties: {
              ...
              dry_run: { type: 'boolean', default: false, description: 'When true, returns the count of memories that would be reaped without deleting them', example: false },
              batch_size: { type: 'integer', default: 1000, minimum: 1, description: 'Maximum number of memories to hard-delete per call (clamped to server-side MEMORY_REAPER_BATCH_SIZE)', example: 500 },
            },
          }, false),
          responses: {
            '200': jsonResponse('Reaper result', {
              ...
              required: ['reaped'],
              properties: {
                reaped: { type: 'integer', description: 'Number of memories permanently deleted (0 when dry_run=true)', example: 12 },
                would_reap: { type: 'integer', description: 'Number of memories that would be reaped (only present when dry_run=true)', example: 42 },
                dry_run: { type: 'boolean', description: 'Whether this was a dry run (only present when dry_run=true)', example: true },
              },
            }),
            ...errorResponses(400, 401, 403, 429, 500),
          },
      ...
      '/memories/digest': {
        post: {
          ...
          description: 'Fetches active memories in a date range from the namespace and clusters them by text similarity. Returns clusters and orphans. Enforces a server-side memory cap to prevent O(n^2) resource exhaustion. Issue #2427/#2439.',
          tags: ['Memories'],
          requestBody: jsonBody({
            type: 'object',
            required: ['since', 'before'],
            properties: {
              since: { type: 'string', format: 'date-time', description: 'Start of the date range (ISO 8601)', example: '2026-03-01T00:00:00Z' },
              before: { type: 'string', format: 'date-time', description: 'End of the date range (ISO 8601, must be after since)', example: '2026-03-13T00:00:00Z' },
              ...
              max_memories: { type: 'integer', default: 500, minimum: 1, description: 'Server-side cap on memories in range (clamped to MEMORY_DIGEST_MAX env)', example: 200 },
              ...
              similarity_threshold: { type: 'number', default: 0.82, minimum: 0, maximum: 1, description: 'Text similarity threshold for clustering (Jaccard word overlap or cosine)', example: 0.82 },
              include_content: { type: 'boolean', default: false, description: 'Whether to include full content in cluster results', example: false },
            },
          }),
          responses: {
            '200': jsonResponse('Digest result', {
              type: 'object',
              required: ['clusters', 'orphans', 'total_memories', 'total_clusters', 'total_orphans'],
              properties: {
                clusters: {
                  type: 'array',
                  description: 'Clusters of related memories (groups >= min_cluster_size)',
                  items: {
                    type: 'object',
                    required: ['id', 'size', 'centroid_text', 'memories', 'avg_similarity', 'time_span'],
                    properties: {
                      id: { type: 'string', description: 'Cluster identifier (e.g. cluster-1)', example: 'cluster-1' },
                      size: { type: 'integer', description: 'Number of memories in this cluster', example: 3 },
                      centroid_text: { type: 'string', description: 'Title of the best representative memory', example: 'User prefers dark mode' },
                      memories: {
                        type: 'array',
                        items: {
                          type: 'object',
                          required: ['id', 'title', 'created_at', 'importance'],
                          properties: {
                            id: { type: 'string', format: 'uuid' },
                            title: { type: 'string' },
                            content: { type: 'string', description: 'Only present when include_content=true' },
                            created_at: { type: 'string', format: 'date-time' },
                            importance: { type: 'number' },
                            similarity: { type: 'number', description: 'Similarity score within cluster' },
                          },
                        },
                      },
                      avg_similarity: { type: 'number', description: 'Average pairwise similarity within the cluster', example: 0.85 },
                      time_span: {
                        type: 'object',
                        required: ['first', 'last'],
                        properties: {
                ...
                orphans: {
                  type: 'array',
                  description: 'Memories that did not cluster with any group',
                  items: {
                    type: 'object',
                ...
                total_memories: { type: 'integer', description: 'Total number of memories in the date range', example: 42 },
                total_clusters: { type: 'integer', description: 'Number of clusters formed', example: 5 },
                total_orphans: { type: 'integer', description: 'Number of unclustered memories', example: 12 },
              },
            }),
            ...errorResponses(400, 401, 403, 429, 500),
          },
      ...
      '/memories/bulk-supersede': {
        post: {
          ...
          summary: 'Atomically mark multiple memories as superseded by a target',
          description: 'Atomically marks all source memories as superseded by the target memory. Uses SELECT FOR UPDATE to prevent concurrent double-supersede. All source and target memories must be in the same namespace. Issue #2429/#2441.',
          tags: ['Memories'],
          requestBody: jsonBody({
            type: 'object',
            required: ['target_id', 'source_ids'],
            properties: {
              target_id: { type: 'string', format: 'uuid', description: 'UUID of the consolidation (target) memory that supersedes the sources', example: 'f1e2d3c4-b5a6-7890-cdef-1234567890ab' },
              source_ids: {
                type: 'array',
                minItems: 1,
                maxItems: 100,
                items: { type: 'string', format: 'uuid' },
                description: 'UUIDs of memories to mark as superseded (must not include target_id)',
                example: ['d290f1ee-6c54-4b01-90e6-d701748f0851', 'a1b2c3d4-e5f6-7890-abcd-ef1234567890'],
              },
              deactivate_sources: { type: 'boolean', default: true, description: 'When true, also sets is_active=false on all source memories', example: true },
            },
          }),
          responses: {
            '200': jsonResponse('Bulk supersede result', {
              type: 'object',
              required: ['superseded', 'target_id'],
              properties: {
                superseded: { type: 'integer', description: 'Number of memories that were superseded', example: 3 },
                target_id: { type: 'string', format: 'uuid', description: 'UUID of the target memory', example: 'f1e2d3c4-b5a6-7890-cdef-1234567890ab' },
              },
            }),
        ...
        put: {
          ...
          requestBody: jsonBody({
            ...
            properties: {
              ...
              importance: { type: 'number', minimum: 0, maximum: 10, description: 'Importance score: 0-1 float (normalized to 1-10) or 1-10 integer', example: 0.6 },
